        ...
        try {
            ...
            if (!res.ok) {
                ...
                if (res.status === 409) {
                    ...
                    if (msg === "A pending request already exists") {
                        setRelationship("request_sent");
                        return;
                    }
                }
                setError(msg || "Could not send request");
                return;
            }
            setRelationship("request_sent");
            onSent?.();
    ...
    if (relationship === "friends") {
        return (
            <span
                className={
                    compact
                        ? "inline-flex items-center justify-center px-3 py-1.5 rounded-lg text-[12px] font-semibold bg-[var(--base-100)] text-[var(--base-600)] ring-1 ring-[var(--base-200)]/80"
                        : "text-[12px] font-medium text-[var(--base-600)]"
                }
                style={{ fontFamily: "var(--font-inter)" }}
            >
                Friends
