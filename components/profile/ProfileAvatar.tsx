...
export function ProfileAvatar({
  ...
  const initialPx = Math.round(size * 0.38);

  return (
    <div
      className={`grid shrink-0 place-items-center rounded-full border border-[var(--base-200)] bg-[var(--primary-50)] text-[var(--primary-600)] transition-[background-color,border-color,transform] duration-200 ease-out ${className}`}
      style={{ width: size, height: size }}
      aria-label={altLabel}
      role="img"
    >
      {/* Grid + tight line-height avoids flex/baseline drift so the letter sits dead-center in the circle */}
      <span
        className="font-bold leading-none select-none"
        style={{ fontSize: initialPx, lineHeight: 1 }}
      >
        {initial}
      </span>
    </div>
